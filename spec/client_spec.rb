...
describe Brio::Client do
  ...
  describe "api requests" do
    ...
    it "should get user's stream if param global not set" do
      stub_get('/stream/0/posts/stream').with(:query => {"count" => 10}).
        to_return(:body => fixture("user_stream.json"), :headers => {:content_type => "application/json; charset=utf-8"})
    ...
    it "should get global stream if param global set" do
      stub_get('/stream/0/posts/stream/global').with(:query => {"count" => 10}).
        to_return(:body => fixture("global_stream.json"), :headers => {:content_type => "application/json; charset=utf-8"})
    ...
    describe "posting to app.net" do
      ...
      it "should tell OK if posting successful" do
        @endpoint.to_return(:body => fixture("post.json"), :headers => {:content_type => "application/json; charset=utf-8"})
        response = @client.post @text
        response.keys.include?('data').should be_true
      end

      it "should fail gracefully if something went wrong" do
        @endpoint.to_return(:body => fixture("error.json"), :headers => {:content_type => "application/json; charset=utf-8"})
        response = @client.post @text
        response.keys.include?('error').should be_true
      end
    end
